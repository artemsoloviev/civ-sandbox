...
export function mountHUD({onEndTurn, onScience}={}){
  ...
  .g-top{position:fixed;left:50%;top:12px;transform:translateX(-50%);z-index:8;text-align:center;
    background:#12233aee;color:#fff;padding:7px 13px;border-radius:14px;box-shadow:0 3px 14px #0006;   /* 08.09: пустые поля по бокам ужаты вдвое, шрифт прежний */
  ...
  /* 07.09: плашка даты сама открывает «Обзор державы» — отдельная плитка в #stats убрана, ряд стал короче */
  .g-top.clickable{cursor:pointer;transition:background .12s,box-shadow .12s,transform .1s}
  .g-top.clickable:hover{background:#1b3556f2;box-shadow:0 5px 20px #0008}
  .g-top.clickable:active{transform:scale(.985)}
  .g-top .yr .emp-ic{display:inline-block;vertical-align:-2px;margin-right:7px;opacity:.92}
  ...
  top.innerHTML=`<div class="yr" id="g-yr"><span id="g-yr-t">—</span></div>
  ...
  return {
    ...
    update(turn){
      ...
      $('g-yr-t').textContent=fmtYear(y);   // 07.09: только текст года — иконка-храм в .yr рядом остаётся на месте
